function expResults = runPanelExperiment(direct, protNums, random)
...
% protNums-vector (optional). List of protocols numbers to use it this
%          experiment. If none is given all protocols are used. 
...
if random 
    ...
    tempOrd = randperm(nProt);
    tempnProt = protNums(tempOrd);
...
for ii=1:nProt
    ...
    sprintf(['Presenting protocol %d (%d of %d):', patName], relInd, ii, nProt) 
...
timeStamp = [timeStamp{:}];
save(fullfile(direct, ['expInfo', timeStamp,]), 'expResults', 'expStruct')
