...
%            initial position based on pattern type (VBars, HBars, SQ16, SQ10, SQSig).  
...
if nargin < 3
    protInds = getProtocolIndices(expStruct, {'type', 'eq', 'short'});
    if ~protInds
...
for ii=1:nProt
    ...
    switch currProt.InitialPosition(1) % these values are define in the beginning of createProtocolScript
        ...
        case -4 
            offset = iniPosSt.SQ10;
        case -5
            offset = iniPosSt.SQSig;
        otherwise
...
save(fullfile(direct, 'results', ['shortExpInfo', timeStamp,]), 'expResults', 'expStruct', 'iniPosSt')
