function expResults = runBaseExperiment(direct, random)
...
protInds = getProtocolIndices(expStruct, {'type', 'eq', 'base'});
...
%     
    ...
    save(fullfile(direct,'results','tempBaseResults.mat'), 'expResults')
    Panel_com('stop')
...
save(fullfile(direct, 'results', ['baseExpInfo', timeStamp,]), 'expResults', 'expStruct')

delete(fullfile(direct, 'results','tempBaseResults.mat'))
Panel_com('all_off')


% Plotting results

allMv = {expStruct.protocol(protInds).Movement};
relMv = unique(allMv);

for ii=1:length(relMv)
    reldirect = relMv{ii};
    relProts = find(cellfun(@(x) strcmp(x, reldirect), allMv));
    posCell = generatePositionCell(0.05, 0.95, 0.05, 0.95, -1, 0.1, length(relProts));
    axh = zeros(1, length(relProts));
    figure
    set(gcf, 'name', ['Movement: ', reldirect])
    for jj=1:length(relProts)
       ...
       tmptit = expStruct.protocol(protInds(relProts(jj))).ObjectBright;
       title(tmptit)
    end
