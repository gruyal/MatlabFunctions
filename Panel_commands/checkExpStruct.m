...
%                       OutputVol - voltage output from panel controller to uniquely label pattern
%
...
protocolFieldsType = [1,2,2,3,2,2,1,1,1,4,4,4,4,1];
rightSize = {1; [1 2]; [1 4]};
...
for ii=1:numProtocols
    ...
    for jj=1:length(protocolFields)
        ...
        elseif protocolFieldsType(jj) == 4
            if ~isstrprop(tempField, 'alpha')
                fprintf(fhand, '%s %s %s\r\n','!!! protocol has wrong value in field', protocolFields{jj}, '!!!');
                errors=errors+1;
            end
        end
