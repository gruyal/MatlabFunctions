function posMatrix = generatePositionCell(startX, endX, startY, endY, gapX, gapY, numPlots)
...
elseif gapX < 0 && gapY >= 0
    ...
    posMatrix = flipud(posMatrix); %so that top plot would be first
...
elseif gapX >= 0 && gapY >= 0
    if size(numPlots) == [1,2]
        ...
        posMatrix = fliplr(posMatrix); %so that top plot would be first
    else
