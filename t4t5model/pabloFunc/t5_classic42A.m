function [V] = t5_classic2A(params,spfr_data)
...
if spfr_data.cat_flag
    ...
    he_fun2 = @(t) exp(-(t-p.stimDur)/p.Tre).*(1-exp(-(p.stimDur)/p.Tre));
    ...
    fe_tmp(s_ind(1):t_ind(2)-1) = fe_fun2(spfr_data.time(s_ind(1):t_ind(2)-1) - p.stimDur);
    ...
    fi_tmp(s_ind(1):t_ind(2)-1) = fi_fun2(spfr_data.time(s_ind(1):t_ind(2)-1) - p.stimDur);
 
    he2_fun1 = @(t) 1-exp(-t/p.Tre2);
    he2_fun2 = @(t) exp(-(t-p.stimDur)/p.Tre2).*(1-exp(-(p.stimDur)/p.Tre2));
    ...
    fe2_tmp(s_ind(1):t_ind(2)-1) = fe2_fun2(spfr_data.time(s_ind(1):t_ind(2)-1) - p.stimDur);
    ...
    for i = 1:size(Ie,1)
        ...
        fe2(logical(p.Ie(i,:)),t_ind(i):(t_ind(i)+size(fe2_tmp,1)-1)) = repmat(fe2_tmp,1,sum(p.Ie(i,:)))';
    end
