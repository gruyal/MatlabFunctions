function [V] = t4_EIEIU(params,spfr_data)
...
if spfr_data.val == 1                   %all params stored in "rows" of 4, 
    ...
    tr_p  = params([19:20,17:18])*10;
    td_p  = params([23:24,21:22])*10;
    tt_p  = params(25:26)*10; %and take tau for second delta
    ti_p  = params([30,29])*10;
end
...
if spfr_data.cat_flag     
    ...
    for i = 1:size(p.Ie,2)
        if any(p.Ie(:,i))
            ...
            t_on = t_ind(counter);
            t_off = s;
            fe(i,:) = fc_sol(t_tot, t_on, t_off, p.Tic, p.Tre, p.Tde);
            fi(i,:) = fc_sol(t_tot, t_on, t_off, p.Tic, p.Tri, p.Tdi);
            fe2(i,:) = fd_sol(t_tot, t_on, t_off, p.Tid, p.Tre2, p.Tde2, p.be2(i));
            fi2(i,:) = fd_sol(t_tot, t_on, t_off, p.Tid, p.Tri2, p.Tdi2, p.bi2(i));        
        end
    end
...
function f = fd_sol(t_tot, t_on, t_off, Ti, Tr, Td, b)
    ...
    I1 = @(t) 1 - exp(-t./Ti); %find init conds
    A = I1(t1);
    B = b;
    C = 0;
