function [V] = t5_EIEIc_joint(params,spfr_data)
...
if spfr_data.cat_flag   
    ...
    %inhibitory (classic)
    hi2_fun1 = @(t) 1-exp(-t/p.Tri2);
 
    if abs(p.Tri2 - p.Tdi2) >= 1e8*(p.Tri2 - p.Tdi2)^2
        E = p.Tri2 - p.Tdi2;
        fi2_fun1 = @(t) 1 - exp(-t/p.Tdi2) - (1/p.Tdi2).*exp(-t/p.Tdi2).*(t + (E*t.^2)./(2*p.Tdi2^2));
        fi2_fun2 = @(t) exp(-t./p.Tdi2).*(fi2_fun1(p.stimDur) + (hi2_fun1(p.stimDur)./p.Tdi2).*(t - E*(t+p.stimDur).*(t)./(p.Tdi2^2)));
    else
        a = (p.Tri2 - p.Tdi2)/(p.Tri2*p.Tdi2);
        fi2_fun1 = @(t) (exp(-t/p.Tdi2)/p.Tdi2).*(p.Tdi2*exp(t/p.Tdi2) - p.Tdi2 - (1/a)*exp(a*t) + (1/a));
        fi2_fun2 = @(t) exp(-t/p.Tdi2).*(fi2_fun1(p.stimDur) + hi2_fun1(p.stimDur)/(a*p.Tdi2)*(exp(a*t) -1));
        if any(isnan(fi2_fun2(spfr_data.time(s_ind(1):t_ind(2)-1)))) || any(isinf(fi2_fun2(spfr_data.time(s_ind(1):t_ind(2)-1))))
            fi2_fun1 = @(t) (exp(-t/p.Tdi2)/p.Tdi2).*(p.Tdi2*exp(t/p.Tdi2) - p.Tdi2 - (1/a)*exp(a*t) + (1/a));
            fi2_fun2 = @(t) exp(-t/p.Tdi2+500).*(fi2_fun1(p.stimDur)*exp(-500) + hi2_fun1(p.stimDur)/(a*p.Tdi2)*(exp((a*t - 500)) -exp(-500)));
        end
    end
    ...
    fi2_tmp(t_ind(1):s_ind(1)-1) = fi2_fun1(spfr_data.time(t_ind(1):s_ind(1)-1));
...
dfe2 = (-fe2' + he2')     / p.Tde2;
dhi2 = (-hi2' + p.stim*p.Ii(p.stim_num,:)) / p.Tri2;
dfi2 = (-fi2' + hi2')     / p.Tdi2;
