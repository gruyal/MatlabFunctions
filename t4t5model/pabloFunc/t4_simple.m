function [V,ge,gi,g_mat,s_mat,num,den] = t4_simple(params,spfr_data)

if spfr_data.val == 1                   %all params stored in "rows" of 4, 
    mu_p  = params(1:4);
    sig_p = params(5:8);
    amp_p = params(9:12);
    tr_p  = params([13,14,13,14])*10;
    td_p  = params([17,18,17,18])*10;
    me_p  = params(23)*.01;
    mi_p  = params(24)*.01;
    be_p  = params(27);
    bi_p  = params(28);
...
if spfr_data.cat_flag     
    ...
    fe2_tmp = fd_sol(t_tot, t_on, t_off, p.Ti, p.Tre2, p.Tde2, p.be2);
    fi2_tmp = fd_sol(t_tot, t_on, t_off, p.Ti, p.Tri2, p.Tdi2, p.bi2);
    ...
    for effDuri = unique(effDur)
        ...
        fe2_tmp(1 + effDuri/p.stimDur,:) = fd_sol(t_tot, t_on, t_off, p.Ti, p.Tre2, p.Tde2, be2);
        fi2_tmp(1 + effDuri/p.stimDur,:) = fd_sol(t_tot, t_on, t_off, p.Ti, p.Tri2, p.Tdi2, bi2);        
    end
