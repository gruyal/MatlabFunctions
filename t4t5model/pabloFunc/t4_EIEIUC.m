function [V] = t4_EIEIUC(params,spfr_data)
...
if spfr_data.val == 1                   %all params stored in "rows" of 4, 
    ...
    del_p = params(21:22)*.01;
    ti_p  = params([26,25])*10;
end
...
if spfr_data.val == 1
    p.be2 = del_p(1); %if PC, deltas are constants.
    p.bi2 = del_p(2); 
else
    p.be2 = effDur*del_p(1); %if NC, linear
    p.bi2 = effDur*del_p(2);
end
...
if spfr_data.cat_flag     
    ...
    for effDuri = unique(effDur)
        ...
        if spfr_data.val == 1
            be2 = del_p(1); %if PC, deltas are constants.
            bi2 = del_p(2); 
        else
            be2 = effDuri*del_p(1); %if NC, linear
            bi2 = effDuri*del_p(2);
        end
