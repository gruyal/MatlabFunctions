function [V] = t4_EIEI_jointA(params,spfr_data)
...
if spfr_data.cat_flag
    ...
    for i = 1:size(p.Ie,2)
        if any(p.Ie(:,i))
            counter = counter+1;
            [~,s] = min(abs(spfr_data.time - spfr_data.time(t_ind(counter)) - effDur(i)));
            fe(i,:) = fc_sol(spfr_data.time,t_ind(counter),s,p.Tre,p.Tde,effDur(i));
            fi(i,:) = fc_sol(spfr_data.time,t_ind(counter),s,p.Tri,p.Tdi,effDur(i));
            fe2(i,:) = fd_sol(spfr_data.time,s,p.Tre2,p.Tde2,p.be2(i));
            fi2(i,:) = fd_sol(spfr_data.time,s,p.Tri2,p.Tdi2,p.bi2(i));        
        end
    end
...
function f = fc_sol(t_tot,t_on,t_off,Tr,Td,effDur)
    %excitatory (classic)
    h_fun1 = @(t) 1-exp(-t/Tr);

    if abs(Tr - Td) >= 1e8*(Tr - Td)^2
        E = Tr - Td;
        f_fun1 = @(t) 1 - exp(-t/Td) - (1/Td).*exp(-t/Td).*(t + (E*t.^2)./(2*Td^2));
        f_fun2 = @(t) exp(-t./Td).*(f_fun1(effDur) + (h_fun1(effDur)./Td).*(t - E*(t+effDur).*(t)./(Td^2)));
    else
        a = (Tr - Td)/(Tr*Td);
        f_fun1 = @(t) (exp(-t/Td)/Td).*(Td*exp(t/Td) - Td - (1/a)*exp(a*t) + (1/a));
        f_fun2 = @(t) exp(-t/Td).*(f_fun1(effDur) + h_fun1(effDur)/(a*Td)*(exp(a*t) -1));
        if any(isnan(f_fun2(t_tot)))
            f_fun1 = @(t) (exp(-t/Td)/Td).*(Td*exp(t/Td) - Td - (1/a)*exp(a*t) + (1/a));
            f_fun2 = @(t) exp(-t/Td+500).*(f_fun1(effDur)*exp(-500) + h_fun1(effDur)/(a*Td)*(exp((a*t - 500)) -exp(-500)));
        end
    end 
    
    f = [zeros(size(t_tot(1:t_on-1)));f_fun1(t_tot(t_on:t_off) - t_tot(t_on));f_fun2(t_tot(t_off+1:end) - t_tot(t_off))]';

end

function f = fd_sol(t_tot,t_off,Tr,Td,b)
    if abs(Tr - Td) >= 1e8*(Tr - Td)^2
        E = Tr - Td;
        f_fun2 = @(t) exp(-t./Td).*(b./Td).*(t - E*(t+effDur).*(t)./(Td^2));
    else
        a = (Tr - Td)/(Tr*Td);
        f_fun2 = @(t) exp(-t/Td).*(b/(a*Td)*(exp(a*t) -1));
        if any(isnan(f_fun2(t_tot)))
            f_fun2 = @(t) exp(-t/Td+500).*(b/(a*Td)*(exp((a*t - 500)) -exp(-500)));
        end
    end 
    
    f = [zeros(size(t_tot(1:t_off-1)));f_fun2(t_tot(t_off:end) - t_tot(t_off))]';
