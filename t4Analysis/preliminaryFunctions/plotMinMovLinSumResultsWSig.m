function varargout = plotMinMovLinSumResultsWSig(minMovLinSumStruct)

% function plotMinMovLinSumResultsWSig(minMovLimSumStruct)
%
% This function plots the results from calcMinMovingBarBasedOnSingleBarSigmoid. 
...
% plots weighted sigmoid sum compared to linear and data
...
allTit = {'PD'; 'ND'; 'PDLin'; 'NDLin'; 'PDwSLin'; 'NDwSLin'};
titComb = [1,2; 1,3; 2,4; 3,4; 1,5; 2,6; 5,6];
...
for ii=1:datSiz(1)
    ...
    for jj=1:datSiz(2)
        ...
        if firNotE
           ...
           plot(minMovLinSumStruct(ii,jj,pdInd(1)).linSum(:,1), minMovLinSumStruct(ii,jj,pdInd(1)).wSigSum, ...
                'linewidth', 3, 'color', pCol(1,:))
        ...
        if secNotE
           ...
           plot(minMovLinSumStruct(ii,jj,pdInd(2)).linSum(:,1), minMovLinSumStruct(ii,jj,pdInd(2)).wSigSum, ...
                'linewidth', 3, 'color', pCol(3,:))
