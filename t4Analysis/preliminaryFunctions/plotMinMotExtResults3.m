function varargout = plotMinMotExtResults3(pStruct)

% function plotMinMotExtResults3(calcMinMotExtSt)
%
        ...
        for jj=1:datSiz(2)        
            ...
            if ~isempty(relDat.linDiff)
                ...
                for kk=1:2 % linDiff is made from first and second bar
                    ...
                    plot(relDat.linSum(diffInds,1), zeros(1,length(diffInds)) + 0.5*(kk-1), 'color', [1,1,1]*0.9 - kk*0.2, 'linewidth', 4)
                    
                    relI = relDat.linDiff(kk).maxResp(1);
                    plot(relDat.linSum(relI, 1), relDat.linDiff(kk).maxResp(2), ...
                         'o', 'markerfacecolor', pCol(2*tt, :), 'markeredgecolor', 'k', 'markersize', 8)
                    
                    relI = relDat.linDiff(kk).maxLinResp(1);
                    plot(relDat.linSum(relI, 1), relDat.linDiff(kk).maxLinResp(2), ...
