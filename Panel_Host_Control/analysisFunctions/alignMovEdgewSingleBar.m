function mvBarSt = alignMovEdgewSingleBar(movEdgeSt, sbSt)
...
%
% INPUT
% movEdgeSt -               moving edge protocol structure 
...
mvPC = movEdgeSt.inputParams.gridCenter; 
...
mvTab = movEdgeSt.gratingTable;
PDFlag = mvTab.orient == pdO;
NDFlag = mvTab.orient == ndO;

grtVal = mvTab.edgeVal + mvTab.bkgdVal; 
grtComb = zeros(length(grtVal), 1); 

for ii=1:length(grtVal)
    switch grtVal(ii)
        case 0.49
            grtComb(ii) = 1; 
        case 1
...
if ismember('grtComb', mvTab.Properties.VariableNames)
    mvTab = [mvTab, table(PDFlag, NDFlag)];
else
    mvTab = [mvTab, table(PDFlag, NDFlag, grtComb)];
end

alignSt = alignProtocolDataByTable(movEdgeSt, 'appear');
...
for ii=1:length(alignSt)
    ...
    tempT = movEdgeSt.gratingTable(ii, :);
