function paramTable = extractFitParameterFromSingleBarFit(allSigFit)

% function extractFitParameterFromSingleBarFit(allSigFit)
% 
% this function extract relevant fit parameters from all the single bar
% fits that are in the same structure (geberated using
% fitAllSingleBarProtocols. 

allNames = fieldnames(allSigFit);

excludeFromAnalysis = {'p20160504_17_24', 'p20160527_12_24'}; % for reasons see fitAllSingleBarProtocols

relNames = setdiff(allNames, excludeFromAnalysis);
...
for ii=1:length(relNames)
    ...
    cellName = repmat(relNames{ii}, datSiz(1)*datSiz(2), 1);
    ...
    tempTable = table(cellNum, cellName, totPos, position, duration, orient, ...
