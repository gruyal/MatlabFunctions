function varargout = plotStimStructRasterByTable2AxesG4(pStruct, figName, axeNames, colName, plotParSt)

% function varargout = plotStimStructRasterByTable2AxesG4(pStruct, figDim, axeNames, colDim, plotParSt)
%
% This function is a modification of plotStimStructByTable2AxesG4. instead of using
...
for ff=1:numFigs
    ...
    if plotSt.figPar(ff).relNumCol < 9 % since set1 has 9 clean colors
        plotSt.figPar(ff).cols = cbrewer('qual', 'Dark2', plotSt.figPar(ff).relNumCol); 
    else
        baseCols = cbrewer('qual', 'Dark2', 8); 
        repCol = ceil(plotSt.figPar(ff).relNumCol/8);
        plotSt.figPar(ff).cols = repmat(baseCols, repCol, 1);
    end
end
