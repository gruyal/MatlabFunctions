...
if isempty(relNames)
    relMatPosIndx = []; % will not plot connected lines
else
    for ii=1:length(relNames)
        relMatPosIndx(ii) = find(strcmp(relNames{ii}, baseTab.Properties.VariableNames));
    end
end
...
for jj=1:numOrt
    ...
    tempPosDat = nan(numGrt+1, length(relMatPosIndx));
    ...
    for ii=1:numGrt
        ...
        for pp=1:length(relMatPosIndx)
            tempIdx = relSt.meanPos(relSt.meanPos(:,2) == relTab.(relMatPosIndx(pp)), 1);
